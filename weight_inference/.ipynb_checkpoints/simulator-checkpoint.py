...
def xpsp_filterer(train, nb_timesteps, timestep, tau_slow, tau_fast):
    ...
    #new code added by Owen
    spike_indices = np.round(train/timestep).astype(int)
    valid_indices = spike_indices[(spike_indices >=0) & (spike_indices < nb_timesteps)]

    if len(valid_indices) > 0:
        #xpsp[train] += 1.0 / (tau_slow - tau_fast)
        xpsp[valid_indices] += 1.0 / (tau_slow - tau_fast)
